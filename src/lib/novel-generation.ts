...
import {
  ...
  generateGeminiJson,
  generateGeminiText,
  generateGeminiTextStream,
} from "@/lib/gemini-provider";
import {
  generateCompatibleCompletion,
  generateCompatibleCompletionStream,
...
async function generateJson(
  ...
  if (settings.provider === "gemini") {
    return generateGeminiJson(settings.gemini.apiKey, prompt, model, schema, temperature);
  }

  return generateCompatibleCompletion(settings.compatible, {
    model,
...
async function generateText(
  ...
  if (settings.provider === "gemini") {
    return generateGeminiText(settings.gemini.apiKey, prompt, model, temperature);
  }
...
async function* generateTextStream(
  ...
  if (settings.provider === "gemini") {
    yield* generateGeminiTextStream(settings.gemini.apiKey, prompt, model, temperature);
    return;
  }

  yield* generateCompatibleCompletionStream(settings.compatible, {
    model,
    prompt,
    temperature,
  });
  ...
  try {
    const rawText = await generateJson(
        ...
        : settings.compatible.outlineModel,
      {
        ...
        properties: {
          synopsis: { type: GeminiSchemaType.STRING },
          worldview: { type: GeminiSchemaType.STRING },
          characterProfiles: { type: GeminiSchemaType.STRING },
          chapters: {
            type: GeminiSchemaType.ARRAY,
            items: {
              type: GeminiSchemaType.OBJECT,
              properties: {
                chapterNumber: { type: GeminiSchemaType.INTEGER },
...
`;
  ...
  if (recentChapters.length > 0) {
    ...
    for (const chapter of recentChapters) {
      context += `- 第${chapter.chapterNumber}章 ${chapter.title}: ${chapter.summary || chapter.synopsis}\n`;
    }
  } else {
